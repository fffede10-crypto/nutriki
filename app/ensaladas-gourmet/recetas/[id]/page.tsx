  ...
  return (
    <div className="min-h-screen pb-28" style={{ backgroundColor: '#FFF8F0' }}>
      <div className="max-w-lg mx-auto">
        ...
        {/* Hero */}
        <div className={`relative h-56 ${CATEGORIA_STYLE[receta.categoria]?.bg || 'bg-green-100'} flex items-center justify-center`}>
          <span className="text-8xl">{CATEGORIA_STYLE[receta.categoria]?.emoji || '🥗'}</span>
          <button
            onClick={() => router.back()}
                ...
                {relacionadas.map((r) => (
                  <Link key={r.id} href={`/ensaladas-gourmet/recetas/${r.id}`} className="w-36 flex-none">
                    <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
                      <div className={`h-20 ${CATEGORIA_STYLE[r.categoria]?.bg || 'bg-green-100'} flex items-center justify-center rounded-t-2xl`}>
                        <span className="text-4xl">{CATEGORIA_STYLE[r.categoria]?.emoji || '🥗'}</span>
                      </div>
                      <div className="p-2">
