...
function FAQItem({ pregunta, respuesta }: { pregunta: string; respuesta: string }) {
  ...
  return (
    <div className="border-b border-gray-200">
      ...
      >
        <span className="font-semibold text-[#1C1917] pr-4">{pregunta}</span>
        <span
...
const trackCheckout = () => { if (typeof window !== 'undefined' && (window as any).fbq) { (window as any).fbq('track', 'InitiateCheckout') } }

const faqs = [
  { pregunta: '¿Para qué edades sirve Nutriki?', respuesta: 'Para niños de 1 a 12 años. Cada receta tiene indicación de edad mínima y los filtros te muestran solo las opciones apropiadas para tu hijo.' },
  { pregunta: '¿Cómo accedo después de comprar?', respuesta: 'Comprás en la tienda, nos escribís por WhatsApp con tu nombre y email, y en menos de 5 minutos te activamos el acceso. Ingresás desde cualquier dispositivo, en cualquier momento.' },
  { pregunta: '¿Las recetas sirven para niños selectivos?', respuesta: 'Sí. Cada receta tiene un "Tip para mamá" con consejos específicos para presentar la comida a niños que rechazan alimentos nuevos. Además tenés la Guía Anti-Selectividad incluida.' },
  { pregunta: '¿Necesito saber cocinar?', respuesta: 'No. Todas las recetas están en pasos simples, con ingredientes básicos y tiempo real de preparación. Si podés hervir agua, podés hacer estas recetas.' },
  { pregunta: '¿Los ingredientes son caros o difíciles de conseguir?', respuesta: 'No. Todos los ingredientes se consiguen en cualquier verdulería o súper del barrio. Sin quinoa, sin semillas importadas, sin nada raro.' },
  { pregunta: '¿Por cuánto tiempo tengo acceso?', respuesta: 'Acceso vitalicio. Pagás una sola vez y la plataforma es tuya para siempre, con todas las actualizaciones y recetas nuevas incluidas sin costo adicional.' },
]
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: '#FFFFFF', color: '#1C1917', minHeight: '100vh' }}>
      ...
      {/* ── HERO (fondo oscuro se mantiene) ── */}
      <div style={{ background: '#0D1F15', color: 'white' }}>
      ...
      {/* ── Social proof bar (verde, se mantiene) ── */}
      <div className="bg-verde py-5 px-4">
        <div className="max-w-2xl mx-auto grid grid-cols-3 gap-2 text-center text-white">
          <div>
      ...
      {/* ── Pain points (blanco) ── */}
      <section className="px-5 py-14 max-w-2xl mx-auto" style={{ background: '#FFFFFF' }}>
        <p className="text-naranja text-xs font-bold uppercase tracking-wider text-center mb-3">¿Te pasa esto?</p>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-center mb-8 text-[#1C1917]">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex gap-4 bg-white rounded-2xl p-4 border-l-4 border-naranja shadow-sm">
              <span className="text-2xl flex-none">{p.ico}</span>
              <div>
                <p className="font-bold text-[#1C1917]">{p.titulo}</p>
                <p className="text-gray-500 text-sm mt-0.5">{p.desc}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-8 rounded-2xl p-6 text-center border border-gray-200 bg-[#FFF8F0]">
          <p className="text-gray-600 text-base leading-relaxed mb-1">No es que no te importa.</p>
          <p className="text-naranja font-bold text-base">Es que no tenés tiempo, ni cabeza, ni ideas nuevas todos los días.</p>
          <p className="text-gray-600 text-base mt-1">Nutriki fue creado exactamente para eso.</p>
        </div>
      </section>

      {/* ── Solución (#F9FAF7) ── */}
      <section className="px-5 py-14 max-w-2xl mx-auto text-center" style={{ background: '#F9FAF7' }}>
        <p className="text-verde text-xs font-bold uppercase tracking-wider mb-3">La solución que estabas buscando</p>
        <h2 className="text-2xl sm:text-3xl font-extrabold mb-4 text-[#1C1917]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="flex flex-col items-center">
              ...
              <p className="text-[#1C1917] font-semibold text-xs text-center mt-2 leading-snug">{b.nombre}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── Resultados (#F9FAF7) ── */}
      <section className="px-5 py-14 max-w-2xl mx-auto" style={{ background: '#F9FAF7' }}>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-center mb-8 text-[#1C1917]">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="bg-white border border-verde/20 rounded-2xl p-4 text-center shadow-sm">
              <span className="text-3xl block mb-2">{r.ico}</span>
              <p className="text-sm font-semibold text-[#1C1917]">{r.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 py-14 max-w-2xl mx-auto" style={{ background: '#FFFFFF' }}>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-center mb-8 text-[#1C1917]">¿Cómo funciona?</h2>
        <div className="space-y-5">
          {[
            ...
            <div key={paso.n} className="flex gap-4 items-start">
              ...
              <div>
                <p className="font-bold text-[#1C1917]">{paso.titulo}</p>
        ...
        <div className="border-2 border-verde rounded-3xl overflow-hidden shadow-md">
          ...
          <div className="px-6 py-8 bg-white">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-500 text-sm mt-1">pago único, acceso vitalicio</p>
            </div>
            <div className="bg-naranja/10 border border-naranja/30 rounded-xl p-3 text-center mb-6">
            ...
            <ul className="space-y-2 mb-6">
              {[
                ...
                <li key={item} className="text-sm text-[#1C1917]">{item}</li>
              ))}
            </ul>
            <a
              href={SHOPIFY_URL}
            ...
            <p className="text-center text-gray-400 text-xs mt-3">🔒 Pago seguro · Mercado Pago</p>
          </div>
        </div>
      </section>

      {/* ── Testimonios WhatsApp (#F0F0F0 simulando app) ── */}
      <section className="px-5 py-14 max-w-2xl mx-auto" style={{ background: '#F0F0F0' }}>
        <h2 className="text-xl sm:text-2xl font-extrabold text-center mb-8 text-[#1C1917]">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
            <div className="px-4 py-3 flex items-center gap-3" style={{ background: '#128C7E' }}>
              <div className="w-8 h-8 bg-verde rounded-full flex items-center justify-center text-sm font-bold text-white">R</div>
              <div>
                <p className="text-white text-sm font-semibold">Romina — Córdoba</p>
                <p className="text-green-200 text-xs">Mamá de Tomás, 5 años</p>
              </div>
            </div>
            <div className="p-4 space-y-1.5" style={{ background: '#ECE5DD' }}>
          ...
          <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
            <div className="px-4 py-3 flex items-center gap-3" style={{ background: '#128C7E' }}>
              ...
              <div>
                ...
                <p className="text-green-200 text-xs">Mamá de Valentina, 7 años</p>
              </div>
            </div>
            <div className="p-4 space-y-1.5" style={{ background: '#ECE5DD' }}>
          ...
          <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
            <div className="px-4 py-3 flex items-center gap-3" style={{ background: '#128C7E' }}>
              ...
              <div>
                ...
                <p className="text-green-200 text-xs">Mamá de Bautista, 3 años</p>
              </div>
            </div>
            <div className="p-4 space-y-1.5" style={{ background: '#ECE5DD' }}>
      ...
      {/* ── Garantía (blanco) ── */}
      <section className="px-5 py-14 max-w-lg mx-auto text-center" style={{ background: '#FFFFFF' }}>
        <div className="text-5xl mb-4">🛡️</div>
        <h2 className="text-2xl font-extrabold mb-3 text-[#1C1917]">Garantía total de 7 días</h2>
        <p className="text-gray-500 mb-2">Si por cualquier motivo no quedás satisfecha,</p>
        <p className="text-verde font-bold text-lg">te devolvemos el 100% de tu dinero.</p>
        <p className="text-gray-500">Sin preguntas, sin vueltas.</p>
      </section>

      {/* ── FAQ (#F9FAF7) ── */}
      <section className="px-5 py-14 max-w-2xl mx-auto" style={{ background: '#F9FAF7' }}>
        <h2 className="text-xl sm:text-2xl font-extrabold text-center mb-8 text-[#1C1917]">Preguntas frecuentes</h2>
      ...
      {/* ── CTA Final (#FFF8F0) ── */}
      <section className="px-5 py-16 text-center max-w-lg mx-auto" style={{ background: '#FFF8F0' }}>
        <h2 className="text-2xl sm:text-3xl font-extrabold mb-3 text-[#1C1917]">
          ¿Seguís pensando qué cocinarle mañana?
        </h2>
        <p className="text-gray-500 mb-8">Más de 800 mamás ya no tienen ese problema.</p>
        <a
          href={SHOPIFY_URL}
          target="_blank"
          rel="noopener noreferrer"
      ...
      {/* ── Footer (#1C1917 oscuro) ── */}
      <footer style={{ background: '#1C1917' }} className="px-5 py-10 text-center">
